...
pub fn update(
    ...
    if on_click {
        let rotate_x_mat =
            glam::Mat3::from_rotation_z(-1.0 * mouse_event_js.get().movement_x as f32 * 0.01);
        ...
        let y_axis = glam::vec3(direction.x, direction.y, direction.z)
            ...
            .normalize();
        let rotate_y_quat =
            glam::Quat::from_axis_angle(y_axis, -1.0 * mouse_event_js.get().movement_y as f32 * 0.01);
