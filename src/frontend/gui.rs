...
fn create_panels() {
            ...
            let panel_grahics_closure: wasm_bindgen::prelude::Closure<dyn FnMut(_)> =
                wasm_bindgen::closure::Closure::wrap(Box::new(move |_event: web_sys::InputEvent| {
                    let graphic_checkbox: web_sys::Element = gloo::utils::document()
                        ...
                        .unwrap();
                    let graphic_checkbox: web_sys::HtmlInputElement = graphic_checkbox.dyn_into().unwrap();
	...
	// Analytics panel
    {
        ...
        panel_analytics_radio
            .set_attribute("type", "checkbox")
            .unwrap();
        panel_analytics_radio.set_attribute("name", "panel").unwrap();
            ...
            let panel_analytics_closure: wasm_bindgen::prelude::Closure<dyn FnMut(_)> =
                wasm_bindgen::closure::Closure::wrap(Box::new(move |_event: web_sys::InputEvent| {
                    let analytics_checkbox: web_sys::Element = gloo::utils::document()
                        ...
                        .unwrap();
                    let analytics_checkbox: web_sys::HtmlInputElement = analytics_checkbox.dyn_into().unwrap();
...
fn create_view_dialog(scene: &std::rc::Rc<std::cell::Cell<engine::update::Scene>>) {
	...
	// graphics view
	{
		...
		// buffer
		{
			...
			let buffer_type_select_element = gloo::utils::document().create_element("select").unwrap();
			...
			let buffer_type_option_render = gloo::utils::document().create_element("option").unwrap();
			buffer_type_option_render.set_text_content(Some("render"));
			let buffer_type_option_normal = gloo::utils::document().create_element("option").unwrap();
	...
	// analytics view
	{
		...
		// device
		{
			...
			let divce_stats_content_element = gloo::utils::document().create_element("div").unwrap();
			...
			device_element
				.append_child(&device_label_element)
				.unwrap();
			device_element
		...
		// fps
		{
			...
			fps_element
				.append_child(&fps_label_element)
				.unwrap();
			fps_element
...
fn reset_state(){
	let all_panel: web_sys::HtmlCollection = gloo::utils::document().get_elements_by_class_name("panel-checkbox");
	...
	let all_dialog: web_sys::HtmlCollection = gloo::utils::document().get_elements_by_class_name("view-dialog");
